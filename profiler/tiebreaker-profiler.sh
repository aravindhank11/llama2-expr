...
run_decorated_inference() {
    ...
    readarray -t forked_pids < <(ps -eaf | grep batched_inference_executor.py | grep -v "${NCU}" | grep -v "nsys" | grep -v "nsight" | grep -v grep | awk '{print $2}')
    if [[ ${#forked_pids[@]} != 1 ]]; then
        echo "Expected 1 batched_inference_executor.py process! Seen != 1..."
        echo "Inspect using command: ' ps -eaf | grep batched_inference_executor.py | grep -v "${NCU}" | grep -v "nsys" | grep -v grep'"
        return 1
    fi
